# Thư viện cần thiết
import streamlit as st 

# Nội dung certifications
def write_certification():
    st.write("""<hr style="border: 1px solid #ccc;">""", unsafe_allow_html=True)

    with st.expander("**1. 📚 Chứng chỉ ngoại ngữ**"):
        st.markdown("""
        + Đạt chứng chỉ B1 Tiếng anh VOCA  
        + Đạt chứng chỉ B1 Tiếng Anh EF-SET  
        + Đạt chứng chỉ B2 Tiếng anh VSTEP  
        """)

    with st.expander("**2. 🎓 Học bổng**"):
        st.markdown("""
        + Đạt học bổng khuyến khích học tập kỳ 1 năm 2021 - 2022  
        + Đạt học bổng khuyến khích học tập kỳ 3 năm 2022 - 2023  
        + Đạt học bổng Đỗ Quân năm học 2022 - 2023  
        + Đạt học bổng Chương trình trọng điểm quốc gia phát triển Toán học giai đoạn 2021 - 2030  
        + Đạt học bổng khuyến khích học tập kỳ 6 năm 2023 - 2024  
        + Đạt học bổng BIDV năm 2024  
        + Đạt học bổng khuyến khích học tập kỳ 7 năm 2024 - 2025  
        """)

    with st.expander("**3. 🌟 Sinh viên 5 Tốt**"):
        st.markdown("""
        + Đạt danh hiệu "Sinh viên 5 Tốt" cấp trường năm 2021 - 2022  
        + Đạt danh hiệu "Sinh viên 5 Tốt" cấp trường năm 2022 - 2023  
        + Đạt danh hiệu "Sinh viên 5 Tốt" tiêu biểu cấp trường năm 2022 - 2023  
        + Đạt danh hiệu "Sinh viên 5 Tốt" cấp ĐHQGHN năm 2022 - 2023  
        + Đạt danh hiệu "Sinh viên 5 Tốt" cấp thành phố Hà Nội năm 2022 - 2023  
        + Đạt danh hiệu "Sinh viên 5 Tốt" cấp trường năm 2023 - 2024  
        """)

    with st.expander("**4. 🏅 Khen thưởng**"):
        st.markdown("""
        + Giấy khen "Gương mặt trẻ tiêu biểu cấp cơ sở" năm 2022  
        + Giấy khen "PCN xuất sắc trong công tác Hội và phong trào sinh viên" năm 2022 - 2023  
        + Giấy khen "Sinh viên học tập tốt và tích cực trong công tác lớp" năm 2022 - 2023  
        + Giấy khen "Sinh viên học tập tốt và tích cực trong công tác lớp" năm 2023 - 2024  
        + Giấy khen "Thành viên tích cực trong CLB MIM Media" năm 2024  
        """)

    with st.expander("**5. 📄 Giấy chứng nhận**"):
        st.markdown("""
        + Đạt GCN đội thi tài năng của cuộc thi "Data Science FTU" năm 2022  
        + Đạt GCN tọa đàm "Hành trình Sinh viên 5 Tốt - Bứt phá để thành công" năm 2022  
        + Đạt GCN cuộc thi "Tự hào màu áo xanh" năm 2023  
        + Đạt GCN thí sinh xuất sắc cuộc thi "Simple Science 2023"  
        + Đạt GCN hội thao "Sinh viên khoẻ" năm 2023  
        + Đạt GCN sinh viên xuất sắc lớp "Cảm tình Đảng" năm 2023  
        + Đạt GCN cuộc thi "Tìm hiểu Tư tưởng Hồ Chí Minh" năm 2023 - 2024  
        + Đạt GCN cuộc thi "Tìm hiểu Văn hóa các nước Đông Nam Á" năm 2023 - 2024  
        + Đạt GCN cuộc thi "Tìm hiểu Nghị quyết Đại hội đại biểu toàn quốc Hội Sinh viên Việt Nam lần thứ XI, nhiệm kỳ 2023 - 2028"  
        + Đạt GCN cuộc thi "Con đường ánh sáng lần VII" năm 2024  
        + Đạt GCN sự kiện "Math Open Day 2024"  
        + Đạt GCN hội thi cấp trường "Olympic tiếng anh lần VI" năm 2024  
        + Đạt GCN hoạt động "Rèn luyện để Bản lĩnh hơn - Triết học MacLenin" năm 2024  
        + Đạt GCN hoạt động "Rèn luyện để Hội nhập tốt hơn" năm 2024  
        + Đạt GCN hội thao "Sinh viên khoẻ" năm 2024  
        """)

    with st.expander("**6. 🏆 Giải thưởng**"):
        st.markdown("""
        + Giải nhất cuộc thi "Lớp tôi là số 1" năm 2023  
        + Giải nhì Hội nghị Khoa học sinh viên Khoa Toán - Cơ - Tin học năm 2025  
        + Giải ba Hội nghị Khoa học sinh viên cấp trường Đại học Khoa học Tự nhiên năm 2025  
        """)

    with st.expander("**7. 🎯 Tốt nghiệp khóa học**"):
        st.markdown("""
        + Khóa học thực chiến "Thiết kế Website với Python Django"  
        + Khóa học thực chiến "Xây dựng hệ thống Website với Java Spring"  
        + Khóa học thực chiến "Lập trình Android với Java và Kotlin"  
        """)